	...
	case 'cantidad_articulos':
		...
		case 'listarArticulos':
			...
			while ($reg=$rspta->fetch_object()) {
				...
				$data[]=array(
	            "0"=>'<button class="btn btn-success btn-xs" id="addetalle" name="addetalle" onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\','.$reg->precio_venta.','.$reg->stock.')"><span class="fa fa-plus"></span> Añadir</button>',
	            "1"=>$reg->nombre,
	            "2"=>$reg->codigo,
	            "3"=>$btncolor,
